...
export function ProjectCertifications({ certifications }: ProjectCertificationsProps) {
  ...
  return (
    <div className="flex flex-col gap-brand-2 rounded-card border border-status-approved/30 bg-status-approved/10 p-brand-3">
      ...
      <ul className="flex flex-wrap gap-brand-2">
        {certifications.map((certification) => (
          <li
            key={certification}
            className="flex items-center gap-brand-1 rounded-data border border-status-approved/30 bg-brand-warm-white px-brand-2 py-1.5 text-data font-medium"
          >
            <BadgeCheck className="size-4 shrink-0 text-status-approved" aria-hidden="true" />
            <Text as="span" className="text-data">
              {certification}
            </Text>
          </li>
        ))}
