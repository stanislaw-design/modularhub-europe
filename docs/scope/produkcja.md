...
- [x] Zbuduj: `/develop nowy układ strony projektu` — kod w `app/[locale]/(customer)/project/[id]/page.tsx`, `components/klient/Project{VariantPicker,CostComparisonTable,RoomLayout,Logistics,Timeline,GalleryTabs}.tsx`, `ProducerCard.tsx`/`ProducerShowcase.tsx` (rozszerzone), `ResultCard.tsx`/`PopularHomeCard.tsx`/`VerifiedManufacturerProjectCard.tsx`/`CompareHomesTeaser.tsx`/`CategoryShowcase.tsx`/`FavoriteCompareTable.tsx` (przełączone z `Project.commercial` na `Project.variants`), `lib/data/{types,projects,producers}.ts`, nowy `lib/data/project-variants.ts` (`getDefaultProjectVariant`, wydzielony bez zależności na `lib/db/client`, żeby komponenty prezentacyjne nie ciągnęły klienta Neon do testów), `lib/db/schema.ts`, migracja `drizzle/0021_public_imperial_guard.sql`, `messages/{pl,en,nl,de}.json`. Typecheck/lint/build i `npx vitest run` czyste (jedyne czerwone testy — `lib/product-family-groups.test.ts` i dwa w `lib/project-request-actions.test.ts` — przedistniejące, niezwiązane z tą funkcją, potwierdzone też w izolacji). `e2e/projekt-szczegoly.spec.ts` NIE zaktualizowany: już dziś martwy niezależnie od tej funkcji (odwołuje się do nieistniejących mockowych id `prj-*`, nie prawdziwych uuid, patrz istniejący Follow-up w scope funkcji 25) — naprawa tego pliku zostaje osobnym zadaniem, nie tym `/develop`.
  - [x] Fundament: migracja addytywna (cztery nowe, opcjonalne kolumny) zastosowana i potwierdzona na żywej bazie Neon, sprzężone przejście `lib/data/projects.ts`/`lib/data/types.ts` i wszystkich odkrytych odbiorców na `Project.variants`/`roomLayout`/`documents`, satisfies AC-1, AC-3, AC-9, AC-10, AC-11
  - [x] Cienki plaster ceny: przełącznik wariantu w hero plus pełna tabela porównawcza kosztów w sekcji "Cena i zakres", satisfies AC-1, AC-2, AC-3, AC-11, AC-14
  - [x] Treść: układ domu, logistyka działki, harmonogram, w potwierdzonej kolejności sekcji, satisfies AC-4, AC-5, AC-6, AC-12, AC-13
  - [x] Zaufanie: zakładki galerii z rozróżnieniem realizacji, rozszerzony `ProducerCard` (odwiedziny, notatka, czas odpowiedzi), satisfies AC-7, AC-8, AC-9, AC-10
