...
- **AC-13**: Każdy standard jest osobnym wariantem (dzisiejszy `product_variant`) z: nazwą, ceną albo jawną flagą "wycena indywidualna" (`priceOnRequest`), i listą pozycji kosztowych (co wchodzi w cenę, co nie, `cost_line_item`, spec 0041). Kształt samej ceny (jedna liczba, nie para min/max) i zniesienie osobnego opisowego pola zakresu (`scopeSummary`/`excludedScope`) na rzecz tej listy jest doprecyzowane w [spec 0051](../0051-cena-od-zamiast-zakresu-wariantu/index.md), która zastępuje tę część tego kryterium.
...
- **AC-24**: *Zastąpione przez [spec 0051](../0051-cena-od-zamiast-zakresu-wariantu/index.md)*. Pierwotnie: opis tego, co nie wchodzi w cenę danego standardu jako osobny, krótki tekst opisowy per standard. Zamiast tego "co nie wchodzi w cenę" wyraża się przez status pozycji kosztowej (AC-13), nie osobne pole tekstowe; katalog z AC-23 pozostaje bez zmian, nadal osobny od pozycji kosztowych.
...
- **AC-28**: Nowy, pojedynczy etap "Tłumaczenia" tłumaczy zatwierdzone treści: opis projektu, nazwy pomieszczeń, własne pozycje sekcji "Co musi zapewnić klient", oraz pytania i odpowiedzi FAQ (`product_translation.faq`, dziś edytowane przez usuwane zakładki, patrz AC-2). Aktywne języki docelowe to angielski, niemiecki i niderlandzki (`en`, `de`, `nl`), źródłem jest polska treść wpisana przez producenta. *Doprecyzowane przez [spec 0051](../0051-cena-od-zamiast-zakresu-wariantu/index.md)*: `scopeSummary`/`excludedScope` nie istnieją już jako pola do tłumaczenia (usunięte razem z `product_variant_translation`); etykiety pozycji kosztowych tłumaczą się osobnym, niezależnym słownikiem (`cost_line_item_label_translation`), nie tym etapem.
...
- **AC-36**: *Zastąpione przez [spec 0051](../0051-cena-od-zamiast-zakresu-wariantu/index.md)*. Pierwotnie: osobny opisowy tekst elementów poza ceną, widoczny obok opisu tego co standard zawiera. Zamiast tego pełna tabela pozycji kosztowych (spec 0041/0045, bez zmian) pozostaje jedynym miejscem tej informacji na karcie projektu klienta.
...
|---|---|---|
| Nowa kolumna `price_on_request` | `product_variant` | boolean, jawna "wycena indywidualna". CHECK pierwotnie `price_on_request = false OR (price_min_cents IS NULL AND price_max_cents IS NULL)` (AC-13, AC-37); przepisany na jedno pole ceny przez [spec 0051](../0051-cena-od-zamiast-zakresu-wariantu/index.md) |
| *Nie dodawane* (zastąpione przez [spec 0051](../0051-cena-od-zamiast-zakresu-wariantu/index.md)) | `product_variant` | pierwotnie planowana kolumna `excluded_scope`; zamiast niej status pozycji kosztowej (`cost_line_item`, bez zmian z spec 0041) wyraża co nie wchodzi w cenę |
| *Nie dodawane* (zastąpione przez [spec 0051](../0051-cena-od-zamiast-zakresu-wariantu/index.md)) | `product_variant_translation` | pierwotnie planowana kolumna `excluded_scope`; cała ta tabela okazała się zbędna i została usunięta razem z `scope_summary` na `product_variant` |
...
| `extractStandardsFromMaterial` | Server Action | `productId`, materiał (`text` \| `table` \| `image` \| `document`) | lista proponowanych wariantów z pewnością, proponowanym dopasowaniem do `completionStandard`, jedną ceną i proponowanymi pozycjami kosztowymi (kształt dokładny w [spec 0051](../0051-cena-od-zamiast-zakresu-wariantu/index.md)) | zły typ lub rozmiar materiału, błąd dostawcy AI, proponowany standard koliduje z już istniejącym aktywnym wariantem (producent łączy albo pomija, AC-15) |
...
1. Migracja addytywna: `price_on_request` na `product_variant` (bez `excluded_scope`, zastąpione przez [spec 0051](../0051-cena-od-zamiast-zakresu-wariantu/index.md)), `client_requirements` na `product` i na `product_translation`, nowa wartość enum `product_sales_pdf` i jej częściowy indeks unikalny, aktualizacja schematów Zod (`technicalSpecs.dom`, kształt `room_layout`). Zweryfikowana na tymczasowej gałęzi Neon. satisfies **AC-13, AC-20, AC-23, AC-25, AC-26, AC-28**
