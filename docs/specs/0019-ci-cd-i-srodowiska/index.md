...
- [x] Załóż drugi trwały branch Neon "staging" (oddzielny od dzisiejszego brancha produkcyjnego z spec 0017) i wygeneruj jego `DATABASE_URL`/`DATABASE_URL_UNPOOLED`. **Dokończone (2026 08 31):** branch `staging` (`br-dawn-tree-b1n3f9lb`) w projekcie Neon `modularhub`; connection stringi (unpooled) pobrane przez Neon MCP i dodane jako sekrety GitHub Actions.
- [x] Dodaj sekrety GitHub Actions: `VERCEL_TOKEN`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`, connection string Neon (staging), connection string Neon (produkcja). Rozważ nazwane GitHub Environments "staging"/"production" czysto jako kontener na te sekrety, bez reguł ochrony (te zostały świadomie odrzucone jako mechanizm bramy, patrz rationale.md Option 3). **Dokończone (2026 08 31):** wszystkie pięć jako Repository secrets: `VERCEL_TOKEN`, `VERCEL_ORG_ID` (`team_YS7WjYys8qIIyBfUyUo3aww8`, przez Vercel MCP), `VERCEL_PROJECT_ID` (`prj_Awqz3aI2M5RPLdf3rOzw9e0tnZTR`, przez Vercel MCP), `STAGING_DATABASE_URL_UNPOOLED`, `PRODUCTION_DATABASE_URL_UNPOOLED`.
