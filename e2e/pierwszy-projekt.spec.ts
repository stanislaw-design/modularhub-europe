...
test.describe("/pl/producent/projekt wizard", () => {
  test("walks all six steps to a read-only summary and saves, clearing the draft (AC-2, AC-3, AC-7, AC-9)", async ({
    page,
  }) => {
    ...
    await expect(page).toHaveURL(/\/pl\/producent\/gotowosc-eksportowa\?nazwa=Modulor\+28/);
    await expect(page.getByRole("heading", { level: 1 })).toHaveText("Projekt „Modulor 28” zapisany");
  ...
  test("shows a generic heading on the export-readiness stub when no project name is given (AC-10)", async ({
    ...
    await expect(page.getByRole("heading", { level: 1 })).toHaveText("Projekt zapisany");
  });
