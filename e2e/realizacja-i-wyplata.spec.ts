...
test.describe("/pl/producent realizacja i wypłata (scope feature 16)", () => {
  ...
  test("full happy path: list → in-progress axis (no button) → delivered axis → verification → confirmation persists on reload", async ({
    ...
    await page.reload();
    await expect(page.getByRole("heading", { level: 2, name: "Modulor Family 90" })).toBeVisible();
    await expect(page.getByRole("heading", { level: 2, name: "Karpaty Alpine 104" })).toBeVisible();

    // In-progress order: 4-stage axis, no verification button yet.
    await page
      .locator("div", { has: page.getByRole("heading", { level: 2, name: "Modulor Family 90" }) })
      .getByRole("link", { name: "Zobacz oś statusu" })
      .first()
      .click();
    await expect(page).toHaveURL(/realizacja\?project=prj-modulor-family-90/);
    ...
    await page.goto("/pl/producent/realizacja?project=prj-karpaty-alpine-104");
    ...
    await expect(page).toHaveURL(/weryfikacja-firmy\?project=prj-karpaty-alpine-104/);
  ...
  test("soft redirects: verification for a non-delivered order, and the axis with no project param", async ({
    ...
    await page.goto("/pl/producent/weryfikacja-firmy?project=prj-modulor-family-90");
    await expect(page).toHaveURL("/pl/producent/realizacja?project=prj-modulor-family-90");
  ...
  test("keyboard pass: one H1 on the verification screen, and submit is reachable and activatable by keyboard", async ({
    ...
    await page.goto("/pl/producent/weryfikacja-firmy?project=prj-karpaty-alpine-104");
