...
export default async function ProducerPanelZapytaniaPage({
  ...
  const selfHref = `/${locale}/producer/panel/inquiries`;
              ...
              {inquiries.map((row) => (
                <tr key={row.id} className="border-b border-brand-steel/50 align-top last:border-b-0">
                  <td className="p-brand-2">
                    <Link href={`/${locale}/producer/panel/inquiries/${row.id}`} className="focus-ring rounded-data font-medium text-brand-passage-blue hover:underline">
