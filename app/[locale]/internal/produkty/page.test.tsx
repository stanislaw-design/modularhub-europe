...
// spec 0031 AC-9: dokładnie ten sam wzorzec auth co /internal/zapytania.
describe("InternalProduktyPage", () => {
  it("redirects to logowanie with the right callbackUrl when there is no session", async () => {
    authMock.mockResolvedValue(null);

    await expect(renderPage()).rejects.toThrow();

    expect(redirectMock).toHaveBeenCalledWith("/pl/logowanie?callbackUrl=%2Fpl%2Finternal%2Fprodukty");
  });

  it("redirects to /klient when the session role is not admin", async () => {
    authMock.mockResolvedValue({ user: { id: "u1", role: "producer" } } as never);

    await expect(renderPage()).rejects.toThrow();

    expect(redirectMock).toHaveBeenCalledWith("/pl/klient");
  });
  ...
  it("renders the product list with photo counts and a link to each product's photo manager for an admin session", async () => {
    ...
    expect(links.map((l) => l.getAttribute("href"))).toContain("/pl/internal/produkty/p1");
